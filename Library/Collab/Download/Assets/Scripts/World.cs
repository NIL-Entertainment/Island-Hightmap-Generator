...
public class World : MonoBehaviour
{
    ...
    public List<BlockData> bds = new List<BlockData>();
    ...
    public byte GetVoxel(Vector3 pos)
    {
        ...
        // Basic Terrain
        int Py = (int)((Noise.GetTerrainPerlin(new Vector2(pos.x,pos.z), 500, biomedata[0,2]) * biomedata[0,1]) + biomedata[0,0]);
        ...
        if (yPos == Py && Py > 50)
        {
            if (Noise.Get2DPerlin(new Vector2(pos.x,pos.z), 0, structuredata[0,0]) > structuredata[0,1])
            {
                //VoxelValue = 4;
                if (Noise.Get2DPerlin(new Vector2(pos.x, pos.z), 123, structuredata[0,2]) > structuredata[0,3])
                {
                    //VoxelValue = 5;
